...
describe("POST /regions/plants/:id/regions/:regionId - link plant to region", () => {
  beforeEach(async () => {
  ...
  it("links a plant to a region as CONTRIBUTOR", async () => {
    const res = await request
      .post(`/regions/plants/${plantId}/regions/${regionId}`)
      .set("Authorization", `Bearer ${contributorToken}`);
...
describe("DELETE /regions/plants/:id/regions/:regionId - unlink plant from region", () => {
  beforeEach(async () => {
    await prisma.plantTag.deleteMany();
    await prisma.plantRegion.deleteMany();
    await prisma.occurrence.deleteMany();
  ...
  it("unlinks a plant from a region as ADMIN", async () => {
    const res = await request
      .delete(`/regions/plants/${plantId}/regions/${regionId}`)
      .set("Authorization", `Bearer ${adminToken}`);
